...
def test_directMapped ( test_name, path="./", verbose=False ):
    ...
    try:
        ...
        # print (e.output)
        print ("Calling ./directMapped returned non-zero exit status.")
...
def grade_directMapped( path='./', verbose=False ):
    ...
    for i in range(2,11):
        if test_directMapped("trace_matMul_{}x{}".format(i,i),path,verbose):
            score += 2
        else:
            break

    print ("Score on directMapped: {} out of 30.".format(score))
    return score
