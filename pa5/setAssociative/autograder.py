...
def generate_test_suite():
    ...
    for i in range(2,11):
        generate_matMul_trace ( i )
        answers_from_csim("trace_matMul_{}x{}".format( i, i ));

def test_setAssociative ( test_name, path="./", verbose=False ):
    ...
    try:
        result = subprocess.run(
            ['./setAssociative', "tests/{}.txt".format(test_name)],
            cwd=path,
            check=True,
            stdout=subprocess.PIPE,
            stderr=subprocess.STDOUT,
...
def grade_setAssociative( path='./', verbose=False ):
    ...
    for filenum in range(4):
        if test_setAssociative("trace_{}".format(filenum),path,verbose):
            score += 3
        else:
            break

    for i in range(2,11):
        if test_setAssociative("trace_matMul_{}x{}".format(i,i),path,verbose):
            score += 2
        else:
            break

    print ("Score on setAssociative: {} out of 30.".format(score))
    return score
