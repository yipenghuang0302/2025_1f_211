...
def test_matTrans ( n, path="./", verbose=False ):
    ...
    try:
        ...
        if verbose:
            ...
            print ("result")
            print (resultlist)
        assert numpy.allclose(resultlist, answer), "The matrix transpose result doesn't match answers/matrix_b_{}x{}.txt.".format(n,n)
        return True
...
def test_cacheOblivious ( n, test_name, path="./", verbose=False ):
    ...
    try:
        ...
        assert result_tallies[1] < answer_tallies[1], "Cache misses need to be less than baseline."
        assert result_tallies[2] < answer_tallies[2], "Cache evictions need to be less than baseline."
        return True
    except subprocess.CalledProcessError as e:
        print (e.output)
        print ("Calling ./cacheOblivious returned non-zero exit status.")
...
def grade_cacheOblivious( path='./', verbose=False ):
    ...
    print ("Score on cacheOblivious: {} out of 30.".format(score))
    return score
